    ...
    saveTimings (group, name, timings) {
      ...
      for (const bridge in timings) {
        if (!device.timings[bridge] || !timingsEqual(device.timings[bridge], timings[bridge])) {
          device.timings[bridge] = timings[bridge]
          dirty = true
          this.warn(`Device ${name} timing for bridge ${bridge} changed from "${JSON.stringify(device.timings[bridge])}" to "${JSON.stringify(timings[bridge])}"`)
        }
      }
